...
const createTypes: { id: CreateType; label: string; detail: string }[] = [
  { id: "expense", label: "Ausgabe", detail: "Ausgabe oder Einnahme erfassen" },
  { id: "task", label: "Aufgabe", detail: "To-do mit Priorität und Deadline" },
  { id: "contract", label: "Vertrag", detail: "Abo, Versicherung oder Frist" },
  { id: "document", label: "Dokument", detail: "HTTPS-Verweis speichern" }
];
...
function ExpenseForm({ categories, labels, today, onSubmit }: { categories: CreateModalProps["categories"]; labels: CreateModalProps["labels"]; today: string; onSubmit: () => void }) {
  ...
  return (
    <form action={createExpense} className="form form-grid modal-form" onSubmit={handleSubmit}>
      ...
      <label>
        Label / Projekt
        <select name="labelId" defaultValue="">
